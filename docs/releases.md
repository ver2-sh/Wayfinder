...
of MSVC linking, Task Scheduler behavior or PowerShell. Similarly, inspecting a
LaunchAgent is not a macOS lifecycle test. Native smoke tests remain release gates;
do them on available local machines rather than triggering hosted builds merely
to prove configuration.
...
process-scoped `ExecutionPolicy Bypass`. Wayfinder refuses automatic Windows
installation unless the existing effective policy is already Bypass,
so it does not relax a user's policy. Checks/notifications still work. Do not
change policy to work around this; use an approved signed/manual installation.
A policy-preserving upstream updater is needed for general Windows one-action
updates under other policies. Downloads trust HTTPS and the release
publisher. dist emits SHA-256 sums; its shell installer verifies embedded archive
checksums. **dist 0.33.0's PowerShell installer does not verify archive hashes**;
